...
## NOTE: Why MeanRatio, not MeanRatio_MoM?? KC: MeanRatio_MoM is a valid alternative, but it's confusing when prop. DNA vs prop. RNA are plotted.
...
## 7.2 Model exploratory plots - under construction
```{r}
...
```{r, fig.cap = "Fig.1D (left) Correlation of mean RNA/DNA ratio in the assay of all amplicons detected in the previral library (n = 407). Dashed line represents RNA/DNA best-fit line of the data (n = 248). Amplicons are colored by whether they were found significant (P < 0.05) using the multiple linear model. (right) Amplicons tested by the multiple linear model (N = 308), with those with significant (FDR < 0.05 and P < 0.05) increase (red, light red) or decrease (blue, light blue) in RNA compared to DNA.", fig.width = 5, fig.height = 4, dpi=300, out.width="70%", out.height="70%"}

# Left

p1 <- ggplot() + 
  geom_point(data = data.all[! data.all$Amp_name %in% data.predict$Amp_name[data.predict$Pvalue < 0.05],], 
...
#ggsave("G:/Shared drives/Nord Lab - Temp Overflow/STARR-408_Lambert&Su-Feher/Draft Figures/Figure #1/FIG1D_AllData_with_lm_KC.svg", 
#       plot = p1, dpi = 300, width = 3, height = 5)

#ggsave("G:/Shared drives/Nord Lab - Temp Overflow/STARR-408_Lambert&Su-Feher/Draft Figures/Figure #1/FIG1D_AllData_with_lm_KC.png", 
#       plot = p1, dpi = 300, width = 3, height = 5)

# Right

# Gradation of data filtering steps:

# 1. data.all = 407 amplicons

# data.clean = 308 amplicons; Passed_QC (Pass_DNA_count, Pass_prop_DNA_and_RNA) & 
#                                                 At_least_1_specific_in_sil_prod
# NOTE: data.clean was used for selecting the training set of 248 amplicons, and 30 + 30 tails

# data.predict = 321 amplicons; Pass_DNA_count, relaxed data.clean set. Edited. Now data.predict = data.clean
...
p2 <- ggplot(fig.1c, aes(x = log2(DNA_prop_mean), y = log2(RNA_prop_mean), color = Sig))+ 
...
# ggsave("G:/Shared drives/Nord Lab - Temp Overflow/STARR-408_Lambert&Su-Feher/Draft Figures/Figure #1/FIG1D_CleanedData_by_P_and_Residual_KCv2.png", 
#       plot = p2, dpi = 300, width = 3, height = 5)

#ggsave("G:/Shared drives/Nord Lab - Temp Overflow/STARR-408_Lambert&Su-Feher/Draft Figures/Figure #1/FIG1D_CleanedData_by_P_and_Residual_KCv2.svg", 
#       plot = p2, dpi = 300, width = 3, height = 5)
...
datatable(
  arrange(data.predict, desc(Residuals_Z_scaled_to_lm))[,c("Amp_name", "Chr", "Start", "End", "MeanRatio", "MeanRatio_sd", "training", "Residuals_Z_scaled_to_lm", "Pvalue", "FDR")], 
  rownames = FALSE)
...
### 7.3.3 Fig. 2A
```{r, fig.cap="Fig. 2A. Bar plot representing distribution of activity in the test assay, sorted by linear model residual. Amplicons are colored as in Figure 1D. MPRA amplicons passing Passed_QC_with_SD; DNA prop > 2^-15 & RNA prop > 2^-15 & DNA count > 200 in all 4 samples; n = 219 amplicons", fig.width = 7, fig.height = 4}
...
```{r, fig.cap = "Fig.2B. The top 18 active amplicons with consistent activity across both linear models and ratiometric models. Three amplicons were used for downstream validation in single-candidate deliveries (6_LD, 3_LD, 161_FBDHS). Bar for all plots represents mean RNA/DNA ratio. Each dot represents individual RNA/DNA ratios for each amplicon (4 dots per amplicon)."}

## Sanity check for reporting amplicon number 
df <- data.predict
df$MeanRatio_m_SD <- df$MeanRatio - df$MeanRatio_sd
#nrow(df) # 308 all amplicons                          
#nrow(filter(df, Pvalue < 0.05)) # 77  Significant amplicons
#nrow(filter(df, Pvalue < 0.05, Residuals_Z_scaled_to_lm > 0)) # 36 positive amplicons
#nrow(filter(df, Pvalue < 0.05, Residuals_Z_scaled_to_lm > 0, MeanRatio_m_SD > 1.5)) # 18 positive amplicons with (MeanRatio - MeanRatio_sd) > 1.5

#nrow(filter(df, MeanRatio_m_SD > 0, MeanRatio > 1.5)) # 71 amplicons with mean ration > 1 SD, and mean ration > 1.5
...
fig.1d.bar <- filter(df, Pvalue < 0.05, Residuals_Z_scaled_to_lm > 0, MeanRatio_m_SD > 1.5)
#dim(fig.1d.bar)

# Color addition is unnecessary
fig.1d.bar$Color <- ifelse(fig.1d.bar$Pvalue >= 0.05, "Non-Significant", "Significant (resd. > 0)")
fig.1d.bar$Color <- ifelse(fig.1d.bar$Pvalue < 0.05 & fig.1d.bar$Residuals_Manual < 0, "Significant (resd. < 0)", fig.1d.bar$Color)

fig.1d.bar <- fig.1d.bar[order(fig.1d.bar$Residuals_Manual, decreasing = F),]
fig.1d.bar$Amp_name <- factor(fig.1d.bar$Amp_name, levels = fig.1d.bar$Amp_name)

fig.1d.bar.melt <- melt(fig.1d.bar[,c("Amp_name", "L1_Activity", "L2_Activity", "L3_Activity", "L4_Activity")], id.vars = "Amp_name")

p <- ggplot(fig.1d.bar) + 
  geom_bar(data = fig.1d.bar, 
           aes(x = Amp_name, y = MeanRatio, fill = Color), 
           stat = "identity", alpha = 0.5, width = 0.75) +
  geom_line(data = fig.1d.bar.melt, 
  ...
  labs(x = "Amplicons",
       y = bquote(Mean(RNA/DNA~Ratio)),
       fill = NULL,
       title = "Amplicons with p < 0.05 and resd. > 0") + 
  theme_bw() + theme(text = element_text(size = 8), 
                     plot.title = element_text(size = 8, hjust = 0.5), 
                     axis.text.x = element_blank(), 
                     axis.ticks.x = element_blank(), 
...
# 8. Epigenomic intersections

## 8.1 Roadmap data download
...
data.predict_w_hg19_coord <- data.predict_w_hg19_coord[,c(2:4, 7:9, 1,5, 10:67)]
colnames(data.predict_w_hg19_coord) <- c("Chr", "Start", "End", colnames(data.predict_w_hg19_coord)[4:66])
...
values(data.predict_w_hg19_coord_GR) <- data.predict_w_hg19_coord[,4:66]

```

## 8.3 Calculate intersections
...
df$Significance <- ifelse(df$P_values_sampling >= 0.05, "Non-significant", "NA")
df$Significance <- ifelse(df$P_values_sampling < 0.05, "P < 0.05", df$Significance)
df$Significance <- ifelse(df$P_values_sampling < 0.01, "P < 0.01", df$Significance)


df_m <- melt(df, id.vars = c("Condition", "Epi_mark", "Epi_peak_q_value", "Fraction_intersecting", "P_values_sampling", "Significance"))
...
df_m$Condition <- ifelse(df_m$Condition == "Low", "Low, P < 0.05", df_m$Condition)
df_m$Condition <- ifelse(df_m$Condition == "High", "High, P < 0.05", df_m$Condition)
df_m$Condition <- ifelse(df_m$Condition == "Middle", "Middle, P > 0.05", df_m$Condition)

# Legend items order
df_m$Condition <- factor(df_m$Condition, levels = c("High", "High, P < 0.05",
                                                    "Middle", "Middle, P > 0.05",
                                                    "Low", "Low, P < 0.05"))

df_m$Significance <- factor(df_m$Significance, levels = c("P < 0.01", "P < 0.05","Non-significant"))
...
if(file.exists("Epi_interst.RData")){
  ...
  load("Epigenomics_intersections/Epi_interst.RData")
...
# Simplifies condition names:
  df_m$Condition <- ifelse(df_m$Condition == "Low, P < 0.05", "Low", as.character(df_m$Condition))
  df_m$Condition <- ifelse(df_m$Condition == "Middle, P > 0.05", "Middle", as.character(df_m$Condition))
  df_m$Condition <- ifelse(df_m$Condition == "High, P < 0.05", "High", as.character(df_m$Condition))


```

## 8.4 Intersections across peak q values
...
## 8.5 Intersections across peak q values
```{r, fig.width = 10, fig.height = 6}
...
df2_m$Condition <- factor(df2_m$Condition, levels = c("Low", "Middle", "High"))
...
### 8.5.1 Fig.2C Intersections at q<0.05
```{r}
...
### 8.5.2 Enrichment P values

Enrichment with permutation test at P < 0.01 and peak q value at 0.05
```{r}

epi_p_table <- distinct(filter(df_m, P_values_sampling < 0.1, Epi_peak_q_value == 0.05)[,c("Condition", "Epi_mark", "Epi_peak_q_value", "Fraction_intersecting", "P_values_sampling", "Significance")])

knitr::kable(arrange(epi_p_table, Epi_mark))

```

# 9. Values reported in the text
...
active_amp_by_lm <- data.predict$Pvalue < 0.05 & data.predict$Residuals_Z_scaled_to_lm > 0
active_amp_by_ratio <- data.predict$MeanRatio > 1.5

# It's more intuitive to run this on amp names instead of Booleans 
active_amp_by_lm <- data.predict$Amp_name[active_amp_by_lm]  # 36
active_amp_by_ratio <- data.predict$Amp_name[active_amp_by_ratio]  #99

setdiff(active_amp_by_lm, active_amp_by_ratio) # "331_GWAS" - is the exception
#setdiff(active_amp_by_ratio, active_amp_by_lm)

# Let's investigate
df <- filter(data.predict, Amp_name %in% c(active_amp_by_lm, active_amp_by_ratio))
filter(df, Amp_name == "331_GWAS")[, c("Amp_name", "MSD", "MeanRatio_MoM", "MeanRatio", "Pvalue", "FDR")]
# "331_GWAS" has negative MSD


# There are actually 35/36 amplicons identified as active using both approaches 

round(35/36, digits = 2)
```
...
sum(data.predict$Pvalue < 0.05 & 
      data.predict$Residuals_Z_scaled_to_lm > 0 &
...
Among 36 positive amplicons with significant regression residual activity (P < 0.05), 18 (50%) had mean ratio – 1 s.d. > 1.5, representing the amplicons with the strongest MPRA-defined enhancer activity (Figure 2B).
